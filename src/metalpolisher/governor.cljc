(ns metalpolisher.governor
  "MetalPolisherGovernor — the independent safety/scope layer gating
  every workshop scheduling/logistics proposal an advisor may make for
  a metal polishing/wheel-grinding/tool-sharpening crew. The governor
  never dispatches hardware itself, never performs grinding or
  polishing work on the shop floor, and never finalizes a
  grinding/polishing-execution decision (e.g. deciding to proceed with
  a specific grinding or polishing operation) or overrides a shop
  ...
  SCHEDULING/LOGISTICS ONLY — it never performs grinding or polishing
  work itself). Modeled on cloud-itonami-isco-7213's
  sheetmetal.governor (and closely on cloud-itonami-isco-9311's
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never performs grinding or polishing
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                grinding/polishing-execution decision
                                (e.g. deciding to proceed with a
                                specific grinding or polishing
                                operation), or to override a shop
                                ...
                                \"proceed with the grinding operation\"
                                / \"authorize the polishing operation\"
                                / \"override the shop safety officer's
                                judgment\", never as bare nouns like
                                \"grinding\", \"polishing\" or
                                ...
                                crew operation for grinding task …\" or
  ...
  regardless of confidence):
    6. :op :flag-safety-concern (an abrasive-wheel-hazard /
                                particulate-exposure / hand-eye-injury
...
(def ^:private scope-excluded-ops
  #{:finalize-grinding-decision :authorize-polishing-operation
    :proceed-with-grinding-work :override-safety-officer-judgment
    :override-shop-safety-officer-judgment})

;; Full finalization/execution ACTION phrases only — never bare nouns
;; ("grinding", "polishing", "wheel", "safety", "shop safety officer")
...
;; "grinding task" / "shop safety officer review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the grinding operation" "proceed with the grinding work"
   "proceed with the polishing operation" "proceed with the polishing work"
   "finalize the grinding decision" "finalize the polishing decision"
   "authorize the grinding operation" "authorize the polishing operation"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :scope-excluded-action
             :detail "研削/研磨工程の実行判断の確定・shop safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `metalpolisher.store/Store`. Pure — never
