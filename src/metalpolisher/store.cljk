(ns metalpolisher.store
  "SSoT for the ISCO-08 7224 metal-polishing/wheel-grinding/tool-sharpening
  workshop scheduling/logistics coordination actor (itonami actor pattern,
  ADR-2607121000 / CLAUDE.md Actors section; README's 'Robotics premise' —
  a workshop scheduling/logistics coordination robot performs crew
  scheduling, task/materials-usage/progress-record logging and
  abrasive-materials/tooling supply-order coordination for a metal
  polishing/grinding/sharpening crew under this advisor/governor pair,
  which never dispatches hardware itself, never performs grinding or
  polishing work itself, and never finalizes a grinding/polishing-execution
  ...
  cloud-itonami-isco-7213's sheetmetal.store (and closely on
  cloud-itonami-isco-9311's mininglabor.store for the
  physical-safety-domain shape).

  Domain:

    worker   — a registered metal polishing/wheel-grinding/tool-sharpening
               crew member (:worker-id, :name)
    workshop — a registered grinding/polishing workshop {:workshop-id :name
