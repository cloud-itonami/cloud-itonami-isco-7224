(ns metalpolisher.actor
  "MetalPolisherActor — the ISCO-08 7224 metal-polishing/wheel-grinding/
  tool-sharpening workshop scheduling/logistics coordination actor as a
  ...
  sheetmetal.actor (and closely on cloud-itonami-isco-9311's
  ...
  The unconditional invariant: the Metal Polisher Advisor can
  never directly commit an operating record the MetalPolisherGovernor
  refuses, and can never make this actor finalize a
  grinding/polishing-execution decision (e.g. a specific grinding or
  polishing operation) or override a shop safety officer's
  ...
  work or authorizes grinding/polishing operations itself."
...
(defn build-graph
  "Build a compiled MetalPolisherActor graph. `store` implements
  `metalpolisher.store/Store`. `advisor` implements
  `metalpolisher.advisor/Advisor` (defaults to `mock-advisor`).
