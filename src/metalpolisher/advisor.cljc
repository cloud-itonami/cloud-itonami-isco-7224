(ns metalpolisher.advisor
  "Metal Polisher Advisor — proposing a workshop scheduling/logistics
  coordination operation (log a work record, schedule a crew
  operation, flag a safety concern, coordinate an abrasive-materials/
  tooling supply order) from a crew roster, workshop registration and
  safety-reporting policy. Swappable mock/llm; the advisor ONLY
  proposes — `metalpolisher.governor` independently gates every
  ...
  grinding/polishing-execution decision (e.g. a specific grinding or
  polishing operation) or to override a shop safety officer's
  judgment — those stay permanently out of this actor's scope.
  Modeled on cloud-itonami-isco-7213's advisor (and closely on
...
(defn- rationale-for [op worker-id workshop-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for grinding task at workshop " workshop-id)
...
(def ^:private system-prompt
  "You are a metal polishing/wheel-grinding/tool-sharpening workshop
   ...
   grinding/polishing-execution decision (e.g. deciding to proceed
   with a specific grinding or polishing operation), or to override a
   ...
   and never performs grinding or polishing work or authorizes
   grinding/polishing operations itself. Safety concerns always
